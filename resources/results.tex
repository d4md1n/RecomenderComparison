...
\paragraph{} Even if RMSE is considered a better metric when large errors are undesired, it is useful to check those systems on the $\frac{MAE}{RMSE}$ metric too. It is easily proven that $MAE \leq RMSE$. Those two are equal when the magnitude of all errors is the same. Examining this ratio we can see if the errors magnitude have close values.
\clearpage
...
\paragraph{}As it was expected and this area of examination, latent factors system has ten percent (10\%) less error spikes than the content based on every dataset. The graph below depicts the results.
\clearpage
...
\paragraph{}Comparing those systems in the execution time metric we can also see that latent factors system outperformed the content base again by taking one third of the time on its worst case.